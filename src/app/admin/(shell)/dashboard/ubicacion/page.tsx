              ...
              <>
                    ...
                    <thead className="border-b border-border text-left">
                      <tr>
                        {["Provincia", "Localidad", "Facturación", "Pedidos", "%"].map((h) => (
                          <th key={h} className="px-4 py-3 text-[11px] font-semibold text-text-secondary">{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {cities.slice(0, 20).map((item) => {
                        ...
                        return (
                          <tr key={`${item.province}-${item.city}`} className="border-b border-border last:border-0">
                            <td className="px-4 py-3 text-[13px] text-text-secondary">{item.province}</td>
                            <td className="px-4 py-3 text-[13px] font-medium text-text-primary">{item.city}</td>
                            <td className="px-4 py-3 text-[13px] text-text-secondary">{formatDashboardPrice(item.revenue)}</td>
                            <td className="px-4 py-3 text-[13px] text-text-secondary">{formatDashboardNumber(item.orders)}</td>
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-2">
                                <div className="h-1.5 w-16 overflow-hidden rounded-full bg-surface-elevated">
                                  <div
                                    className="h-full rounded-full bg-text-secondary/50"
                                    style={{ width: `${Math.max(share, 0)}%` }}
                                  />
                                </div>
                                <span className="text-[12px] text-text-secondary">{formatDashboardPercent(share)}</span>
                              </div>
                            </td>
                          </tr>
                        );
                ...
                <div className="grid gap-3 md:hidden">
                  {cities.slice(0, 10).map((item) => (
                    <div key={`${item.province}-${item.city}`} className="rounded-[10px] border border-border bg-surface-elevated px-4 py-4">
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          <p className="text-[13px] font-semibold text-text-primary">{item.city}</p>
                          <p className="text-[11px] text-text-secondary">{item.province}</p>
                        </div>
                        <p className="shrink-0 text-[13px] font-semibold text-text-primary">{formatDashboardPrice(item.revenue)}</p>
                      </div>
                      <p className="mt-2 text-[11px] text-text-secondary">{formatDashboardNumber(item.orders)} pedidos</p>
                    </div>
                  ))}
                </div>
