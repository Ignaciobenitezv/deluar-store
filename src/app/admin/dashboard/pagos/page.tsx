...
export default async function AdminDashboardPaymentsPage({
  ...
  const totalPaymentOrders = metrics.payments.methods.reduce(
  ...
  const failedOrders =
    metrics.payments.statusBreakdown.find((item) => item.status === "REJECTED")?.orders ?? 0;
  const cancelledOrders =
    metrics.payments.statusBreakdown.find((item) => item.status === "CANCELLED")?.orders ?? 0;
    ...
    >
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <KpiCard
          ...
          value={formatDashboardPrice(approvedRevenue)}
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.2fr)_minmax(0,0.8fr)]">
          ...
          emptyState={
            paymentMethods.length === 0 ? (
              <EmptyState
                ...
                description="Todavía no hay pedidos con método de pago en este período."
              />
            ) : undefined
          }
        >
                  ...
                  {paymentMethods.map((item) => (
                    <tr key={item.method} className="border-t border-slate-200/70">
                      ...
                      <td className="px-4 py-4 text-slate-700">
                        {formatDashboardPercent((item.orders / totalPaymentOrders) * 100)}
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <RankingCard
          ...
          emptyState={
            <EmptyState
              title="Sin facturación por método"
              description="Todavía no hay facturación asociada a métodos de pago en este período."
            />
          }
        />
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]">
        <ChartCard
          ...
          emptyState={
            metrics.payments.statusBreakdown.length === 0 ? (
              <EmptyState
                title="Sin estados de pago"
                description="No hay estados de pago para mostrar en este período."
              />
            ) : undefined
          }
        >
        ...
        <ChartCard
          ...
          description="Señales operativas del período."
        >
          {metrics.conversion.pendingOrders > 0 ||
          metrics.conversion.failedOrders > 0 ||
          failedByMethod.length > 0 ? (
            <div className="space-y-3">
              <div className="grid gap-3 sm:grid-cols-3">
                ...
                <StatBadge
                  label="Fallidos"
                  value={formatDashboardNumber(metrics.conversion.failedOrders)}
                  tone="failed"
                />
                <StatBadge
                  label="Cancelados"
                  value={formatDashboardNumber(metrics.conversion.cancelledOrders)}
                  tone="neutral"
                />
              </div>
              ...
              <div className="rounded-[20px] border border-slate-200/70 bg-slate-50 p-5">
                ...
                <div className="mt-4 space-y-3">
                  {failedByMethod.slice(0, 5).map((item) => (
                    <div
                      key={item.method}
                      className="rounded-[16px] border border-slate-200/70 bg-white px-4 py-3"
                    >
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          <p className="text-sm font-medium text-slate-900">{item.label}</p>
                          <p className="mt-1 text-xs text-slate-500">
                            {formatDashboardNumber(item.pendingOrders)} pendientes ·{" "}
                            {formatDashboardNumber(item.failedOrders)} fallidos
                          </p>
                        </div>
                        <StatBadge
                          label="Fallos"
                          value={formatDashboardNumber(item.failedOrders)}
                          tone="failed"
                        />
                      </div>
                    </div>
