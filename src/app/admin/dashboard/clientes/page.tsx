...
export default async function AdminDashboardCustomersPage({
  ...
  const averageRevenuePerCustomer =
    metrics.customers.uniqueCustomers > 0
      ? metrics.summary.billingTotal / metrics.customers.uniqueCustomers
      : 0;
  ...
  return (
    ...
    >
          ...
          emptyState={
            topCustomerRows.length === 0 ? (
              <EmptyState
                ...
                description="Todavía no hay clientes con compras en este período."
              />
            ) : undefined
          }
        >
                    ...
                    return (
                      <tr key={customer.email} className="border-t border-slate-200/70">
                        ...
                        <td className="px-4 py-4 text-sm text-slate-700">
                          {formatDashboardNumber(customer.orders)}
                        </td>
                        <td className="px-4 py-4 text-sm font-semibold text-slate-950">
                          {formatDashboardPrice(customer.revenue)}
                        </td>
                        <td className="px-4 py-4 text-sm text-slate-700">
                          {formatDashboardPrice(averageTicket)}
                        </td>
                      </tr>
                    );
                  })}
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
        <ChartCard
          ...
          emptyState={
            metrics.summary.paidOrders === 0 ? (
              <EmptyState
                title="Sin ticket promedio por cliente"
          ...
          {metrics.summary.paidOrders > 0 ? (
            <div className="rounded-[20px] border border-slate-200/70 bg-slate-50 p-5">
              ...
              <p className="mt-2 text-sm leading-6 text-slate-500">
                Se toma la facturación total del período y se divide por la cantidad de clientes
                únicos para dar una lectura comercial simple y útil.
              </p>
        ...
        <ChartCard
          title="Segmentación rápida"
          description="Lectura operativa simple sobre el período."
        >
