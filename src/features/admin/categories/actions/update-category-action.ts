...
import { AdminCategoryNotFoundError, AdminCategorySlugTakenError, updateAdminCategory } from "../server/admin-category-service";
...
export async function updateCategoryAction(
  ...
  if (!id) {
    return { status: "error", message: "No encontramos la categoría a editar." };
  }
  ...
  try {
    const node = await updateAdminCategory(id, parsed.data);
    return { status: "success", message: "Categoría actualizada.", node };
  } catch (error) {
    ...
    logger.error("admin.category.update_failed", { id, error: error instanceof Error ? error.message : String(error) });
