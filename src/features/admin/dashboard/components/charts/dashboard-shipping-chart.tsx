...
type DashboardShippingChartProps = {
  data: ShippingMethodPoint[];
};

const shippingColors = ["#314158", "#3b82f6", "#10b981", "#f59e0b", "#f43f5e"];

function ShippingDonut({ data }: { data: ShippingMethodPoint[] }) {
          ...
          <PieChart margin={{ top: 12, right: 18, bottom: 12, left: 18 }}>
            <Tooltip
              content={({ active, payload }) => {
                const point = payload?.[0]?.payload as ShippingMethodPoint | undefined;
            ...
            >
              {data.map((entry, index) => (
                <Cell key={entry.method} fill={shippingColors[index % shippingColors.length] ?? dashboardChartColors.navy} />
              ))}
            </Pie>
          </PieChart>
        </ResponsiveContainer>
...
function ShippingLegend({ data }: { data: ShippingMethodPoint[] }) {
  const totalOrders = data.reduce((accumulator, item) => accumulator + item.orders, 0);
...
export function DashboardShippingChart({ data }: DashboardShippingChartProps) {
    ...
    <div ref={ref} className="w-full min-w-0">
      {useSideLegend ? (
        <div className="grid gap-4 grid-cols-[minmax(0,1fr)_minmax(0,190px)]">
          <ShippingDonut data={visibleData} />
          <ShippingLegend data={visibleData} />
        </div>
      ) : (
